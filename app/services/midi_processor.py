...
class MidiProcessorService:
    ...
    ) -> Tuple[SongResponse, Dict[str, Any]]:
        """Convert MIDI file to Song object"""
        try:
            pm = pretty_midi.PrettyMIDI(file_path)
            
            # Find the best instrument (usually the one with most notes)
            ...
            if not best_instrument:
                raise ValueError("No suitable instrument found in MIDI file")
            ...
            for note in best_instrument.notes:
                ...
                if octave < options.min_octave or octave > options.max_octave:
                    continue
                
                raw_notes.append({
                    "key": note_name,
                    "start_time": int(note.start * 1000),
                    "duration": max(int((note.end - note.start) * 1000), options.min_note_duration)
                })
            
            if not raw_notes:
                raise ValueError(f"No notes found in octave range {options.min_octave}-{options.max_octave}")
            
            # Sort by start time
            raw_notes.sort(key=lambda x: x["start_time"])
            
            # Apply processing options
            processed_notes = self._process_notes(raw_notes, options)
            
            if not processed_notes:
                raise ValueError("No notes remaining after processing")
            
            # Convert to NoteSchema objects
            notes = [NoteSchema(**note) for note in processed_notes]
